...
@media screen and (min-width:1px) and (max-width: 500px) {
  ...
  <div class="taskbar">
    ...
    <a href="mypage.php"><img class="icon_mypage" src="8-account-solid.png" /> </a> 
...
<div class="posts">
  ...
  $publish = $database->prepare("SELECT * FROM posts WHERE data_id= $id");
  $publish ->execute();
  ...
  foreach ($my_data AS $data) {
      $getmyphoto="data:".$data['type_photo'].";base64,".base64_encode($data['file']);
      echo '<br><img class="personal_photo" src="'.$getmyphoto.'">' ;
      echo '<a href="chang_my_photo.php"><img class="change_my_photo" src="18-autorenew-outline.png"/></a>';
      echo "<h3>".$data['f_name'].$data['l_name']."</h3>";
      
  }
  //check number of chat
  $check=$database->prepare("SELECT * FROM `end_chat` WHERE `my_id` = $id ");
  $check->execute();
  $count=$database->prepare("SELECT * FROM `chat` WHERE `id_recive` = $id ");
  $count->execute();
  $var=$count->rowCount();
  foreach ($check as $num) {
      ...
      //friends ifollow and follow me and massege
      echo '<form method="POST" enctype="multipart/form-data"><button type="submit" class="buttons" name="ifollow">I Follow</button><button type="submit" class="buttons" name="followme">Follow Me</button><button type="submit" class="buttons" name="massege"><img class="imgmassege" src="192-forum-outline.png" />'.$number.'</button></form>';
      if (isset($_POST['ifollow'])) {
          header("location: myfriend.php");
          ob_end_flush();
      }
      if (isset($_POST['followme'])) {
          header("location: followme.php");
          ob_end_flush();
      }
      if (isset($_POST['massege'])) {
         $update=$database->prepare("UPDATE `end_chat` SET `end_number`=$var WHERE my_id =$id" );
         $update->execute();
        ...
        if ($result['name']=="") {
          ...
          //increese like
            ...
            if (isset($_POST['l'.$result["id"]])) {
              ...
              if ($search->rowCount()!==1&&$get->rowCount()!==1) {
                  $getlike = $database->prepare("SELECT * FROM posts WHERE id= $id_like  ");
                  $getlike ->execute();
                  $addlike=$database->prepare("INSERT INTO `like` ( `post_id`, `client_id`) VALUE(:id_post,:id)");
          ...
          if (isset($_POST['d'.$result["id"]])) {
            ...
            if ($get->rowCount()!==1&&$search->rowCount()!==1) {
                  $getlike = $database->prepare("SELECT * FROM posts WHERE id= $id_like  ");
                  $getlike ->execute();
                  $adddlike=$database->prepare("INSERT INTO `d_like` ( `post_id`, `client_data`) VALUE(:iid_post,:iid)");

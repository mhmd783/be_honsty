...
<?php
    ...
    if(isset($_POST['notfans'])){
        ...
        $puplish_fans=$database->prepare("SELECT * FROM `comment` WHERE `id_who_have_post`=$id ORDER BY `id` DESC" ); 
        $puplish_fans->execute();
        foreach($puplish_fans AS $fans){
            $client_id=$fans['client_like_id'];
            $post=$fans['posts_id'];
            ...
            foreach($client AS $data){
                ...
                echo '<img class="photo_commenter" hight="10%" width="10%" src="'.$getphotoclient.'"/><img class="imglike"src="icons/47-chat-outline.png"/><form method="POST" enctype="multipart/form-data"><button type="submit" class="fbuttons" name="b'.$post.'"><h3 class="name_client">'.$data['f_name'].$data['l_name'].'</h3></button><input type="hidden" value="'.$post.'" name="c'.$post.'"/></form>'  ;
                if (isset($_POST['b'.$post])) {
                    $id_anither_client=$_POST['c'.$post];
